...
class MethodConformanceTestCase(APITestCase):
    ...
    def test_2(self):
        ...
        self.account1_priorities1 = PrioritiesModelFactory(account=self.account1, priorities_name=self.priorities1,
                                                           importance=10)
        self.account2_priorities1 = PrioritiesModelFactory(account=self.account2, priorities_name=self.priorities1,
                                                           importance=10)
        self.account3_priorities1 = PrioritiesModelFactory(account=self.account3, priorities_name=self.priorities1,
                                                           importance=5)
        request_data = {}
        url = reverse('priorities:prioritiesmodel-compatibility-list')
        response = self.client.get(url, request_data, format='json')
